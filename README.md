...
右键托盘图标可以打开菜单，支持切换小窗/大窗、置顶、开机自启动、更换配色、切换深色模式、设置自动刷新秒数、检查更新、重启、查看版本号和退出。
...
大窗会显示刷新时间、重置时间和两条进度条。配色只影响正常额度档的主色调，可选红、橙、黄、绿、青、蓝、紫、黑、白；低额度时仍按原逻辑显示橙色和红色。深色模式会把背景切换为黑色、普通文字切换为白色，进度条和百分比仍保留额度档位颜色逻辑。

工具默认每 30 秒自动刷新一次。可在托盘或弹窗右键菜单的“自动刷新”子菜单中选择 `30s`、`1min`、`5min`、`10min`、`20min`、`30min`、`60min`，也可以点击窗口刷新图标手动刷新。

启动时会自动检查 GitHub Release 是否有新版本；如果有更新，任务栏托盘图标会显示小红点。下载更新时，小窗会显示下载进度；正式安装版下载完成后会启动安装程序，便携版会让用户选择保存目录。
...
- 启动时自动检查新版本，有更新时托盘图标显示小红点。
- 右键菜单增加“检查更新”。
- 更新下载时大窗和小窗同步显示进度。
- 正式安装版下载完成后启动安装程序；便携版下载完成后由用户选择保存位置。

### 2.2.3

- 大窗额度进度条和百分比数字改为发光渐变样式，配色只影响正常档主色调，橙色/红色警告逻辑保留。
- 刷新失败时保留最后一次成功额度，并在状态栏显示失败原因。
- 自动刷新预设调整为 `30s`、`1min`、`5min`、`10min`、`20min`、`30min`、`60min`。
- 右键菜单增加配色选择和深色模式切换。
